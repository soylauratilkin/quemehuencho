  ...
  if (loading) {
    return (
      <div className="min-h-dvh bg-[#0a0a0a] flex items-center justify-center">
        <div className="text-[#ff751f] text-xl font-bold">Cargando dashboard...</div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-dvh bg-[#0a0a0a] pb-20">
      ...
      <header className="sticky top-0 z-10 flex items-center gap-3 border-b border-[#333] bg-[#0a0a0a]/95 px-4 py-4 backdrop-blur">
        <button onClick={() => router.push("/admin/pedidos")} className="flex size-10 items-center justify-center rounded-full bg-[#1a1a1a] text-white">
          ...
          {(["hoy", "semana", "mes"] as const).map((p) => (
            <button
              ...
              className={`flex-1 rounded-xl py-2 text-sm font-bold capitalize transition-all ${
                periodo === p ? "bg-[#ff751f] text-black" : "text-gray-400"
              }`}
            >
              {p}
            </button>
      ...
      <div className="space-y-4 px-4">
        ...
        {data && (
          <div className="rounded-3xl bg-gradient-to-br from-[#ff751f] to-[#ff751f]/80 p-6 text-black shadow-xl">
            ...
            <div className="text-4xl font-extrabold">{formatPrice(data.promedioDiario)}</div>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <MetricCard
            title="Por Día"
            value={data?.totalHoy && data?.pedidosHoy ? Math.round(data.totalHoy / data.pedidosHoy) : 0}
        ...
        {data && (
          <div className="rounded-3xl bg-[#111] p-5 ring-1 ring-[#333]">
            ...
            <div className="space-y-3">
              <TypeRow icon={<Bike className="size-4" />} label="Delivery" value={data.porTipo.delivery} color="bg-orange-500" />
              <TypeRow icon={<Store className="size-4" />} label="Retiro" value={data.porTipo.retiro} color="bg-blue-500" />
              <TypeRow icon={<Car className="size-4" />} label="Doblefila Express" value={data.porTipo.doblefila} color="bg-green-500" />
              <TypeRow icon={<Users className="size-4" />} label="Mesa" value={data.porTipo.mesa} color="bg-purple-500" />
            </div>
          </div>
        )}
...
function TypeRow({ icon, label, value, color }: any) {
  const total = 100 // Esto debería ser el total real, pero por ahora es estático
